...
--
-- Host: localhost
-- Generation Time: May 28, 2016 at 03:13 PM
...
CREATE TABLE IF NOT EXISTS `barang_keluar_mentah` (
  ...
  PRIMARY KEY (`id`),
  KEY `id_masuk_barang_UNIQUE` (`id_masuk_barang`)
...
CREATE TABLE IF NOT EXISTS `detile_proses_2` (
  ...
  `keterangan` varchar(1024) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `detile_proses_2_proses_idx` (`id_proses_2`),
  KEY `detile_proses_2_barang_keluar_idx` (`id_keluar_barang`)
...
CREATE TABLE IF NOT EXISTS `proses_2` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `id_proses_1_UNIQUE` (`id_proses_1`)
) ENGINE=InnoDB  DEFAULT CHARSET=latin1 AUTO_INCREMENT=3 ;
