<?php

namespace app\modules\gudang\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\modules\gudang\models\MasukBarang;

/**
 * MasukBarangSearch represents the model behind the search form about `app\modules\gudang\models\MasukBarang`.
 */
class MasukBarangSearch extends MasukBarang
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['id_masuk', 'id_pembelian'], 'integer'],
            [['kode_masuk', 'tanggal_masuk', 'keterangan'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = MasukBarang::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
            'pagination'=>[
                'pageSize'=>5
            ]
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id_masuk' => $this->id_masuk,
            'id_pembelian' => $this->id_pembelian,
            'tanggal_masuk' => $this->tanggal_masuk,
        ]);

        $query->andFilterWhere(['like', 'kode_masuk', $this->kode_masuk])
            ->andFilterWhere(['like', 'keterangan', $this->keterangan]);

        return $dataProvider;
    }
}
